...
public class Task16 {
    public static void main(String[] args) {
        ...
        if (n % 2 != 0) {
            magicSquare = Task16.oddArray(n);
        }
        if (n % 2 == 0 && n % 4 != 0) {
            magicSquare = Task16.evenOddArray(n);
        }
        if (n % 4 == 0) {
            magicSquare = Task16.simpleSquare(n);
            Task16.removeDiagonals(magicSquare);
            Task16.fillEmptyElements(magicSquare);
        }
        Task16.printArray(magicSquare);

    }

    public static int[][] oddArray(int n) {
        int[][] magicSquare = new int[n][n];
        int[][] tempSquare = Task16.simpleSquare(n);
        int[][] columns;
        int[][] rows;
    ...
    public static int[][] evenOddArray(int n) {
        ...
        int[][] tempMagicSquare = Task16.oddArray(tempN);
        for (int i = 0; i < tempN; i++) {
